...
2. Create and store shared data
	* Service are a good place to store data. Angular only instantiates a single instance of each service for a given application. 
	* For example, say you have 3 different Controllers which are not all on the same screen, referencing the `$log` service. The Controllers will get the same `$log` service instance, therefore the same shared data.
3. Manage complexity/separation of concerns

### Demo:
Build a custom service to house the complexity of the GitHub API calls. Get the  
